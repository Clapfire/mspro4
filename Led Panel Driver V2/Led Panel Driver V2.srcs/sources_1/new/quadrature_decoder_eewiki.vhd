...
ENTITY quadrature_decoder IS
	GENERIC(
--		positions						:	INTEGER := 256; 		--size of the position counter (i.e. number of positions counted)
		debounce_time					:	INTEGER := 100_000; 	--number of clock cycles required to register a new position = debounce_time + 2
		set_origin_debounce_time	:	INTEGER := 1_000_000);	--number of clock cycles required to register a new set_origin_n value = set_origin_debounce_time + 2
	PORT(
		...
		position			:	OUT UNSIGNED(7 downto 0));	--current position relative to index or initial value
...
ARCHITECTURE behavioral OF quadrature_decoder IS
	...
	SIGNAL position_sig : unsigned (7 downto 0);
	
	signal a_inv : std_logic;
    signal b_inv : std_logic;
    signal btn_inv : std_logic;
...
BEGIN
	...
	BEGIN
		IF(clk'EVENT AND clk = '1') THEN													--rising edge of system clock
			...
			a_new <= a_new(0) & a_inv;																--shift in new values of 'a'	
			b_new <= b_new(0) & b_inv;																--shift in new values of 'b'
			...
			set_origin_n_new <= set_origin_n_new(0) & btn_inv;					--shift in new values of set_origin_n	
			...
			IF(set_origin_n_int = '0') THEN														--inital position is being set
				position_sig <= "00000000";																				--clear position counter
